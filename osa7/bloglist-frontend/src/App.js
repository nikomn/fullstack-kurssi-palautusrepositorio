...
const App = () => {
  ...
  const createBlog = async (blog) => {
    try {
      ...
      const notification = { message: `a new blog '${newBlog.title}' by ${newBlog.author} added!`, type: 'success', cue: 1 }
      dispatch(setReducerNotification(notification, 5))
    } catch(exception) {
  ...
  const handleLike = async (id) => {
    ...
    const notification = { message: `added like to '${blogToLike.title}'`, type: 'success', cue: 1 }
    console.log('uusi notification: ', notification)
    dispatch(setReducerNotification(notification, 5))
  }
